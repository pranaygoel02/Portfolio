...
export function CMDKProvider({ children }) {
    ...
    useEffect(() => {
      async function fetchData() {
        try {
          // const response = await fetch(`https://pranaygoel-server.vercel.app/cmdk`);
          // const data = await response.json();
          dispatch({ type: actions.SET_DATA, payload: CMDK_DATA });
        } catch (err) {
          console.log(err);
        }
      }
        ...
        if (matchingCommand) {
          ...
          switch (actionToPerform) {
            ...
            case "push_to_action_stack":
              openCommandPalette();
              console.log(filteredData[selectedItem]);
              dispatch({
                type: actions.SET_ACTION_STACK,
                payload: filteredData?.[selectedItem]?.stack,
              });
              break;
            default:
              break;
